...
public class VacancySQLDataProvider {
    ...
    public static List<Vacancy> getEmployeeVacanciesList(Employee employee) {
        ...
        EmployeeSQLDataProvider.setEmployeeIdForEmployee(employee);
        ...
        employee.setVacancies(vacanciesList);
        return vacanciesList;
    }

    public static String getEmployeeIdByVacancy(Vacancy vacancy) {

        String employeeId = "";

        String query = String.format("SELECT  %s FROM %s WHERE %s='%s' ",
                VacancyColumnsName.HIRING_MANAGER_ID.getName(),
                TableName.VACANCY_TABLE.getName(), VacancyColumnsName.NAME.getName(),
                vacancy.getName());
